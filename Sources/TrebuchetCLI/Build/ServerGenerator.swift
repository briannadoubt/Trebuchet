...
public struct ServerGenerator {
    ...
    func generate(
        ...
        let projectName = try detector.getPackageName() ?? config.name

        // Copy actor sources if it's an Xcode project
        if detector.isXcodeProject && !detector.hasPackageSwift {
            terminal.print("  Detected Xcode project, copying actor sources...", style: .dim)
            let actorSourcesPath = "\(outputPath)/Sources/ActorSources"
            try FileManager.default.createDirectory(
                atPath: actorSourcesPath,
                withIntermediateDirectories: true
            )
            try detector.copyActorSources(
                actors: actors,
                to: actorSourcesPath,
                verbose: verbose,
                terminal: terminal
