...
public struct DockerBuilder {
    ...
    public func build(
        ...
        let buildDir = "\(projectPath)/.trebuche/build"
        ...
        try await runDocker(
            ["build", "-t", "trebuche-build-\(config.projectName)", "-f", dockerfilePath, projectPath],
            verbose: verbose
        )

        // Extract binary from container
        terminal.print("  Extracting binary...", style: .dim)
        let containerName = "trebuche-extract-\(UUID().uuidString.prefix(8))"

        // Create container
        try await runDocker(
            ["create", "--name", containerName, "trebuche-build-\(config.projectName)"],
            verbose: verbose
        )
    ...
    private func generateDockerfile(projectName: String) -> String {
        ...
        COPY .trebuche/build/bootstrap.swift /app/
